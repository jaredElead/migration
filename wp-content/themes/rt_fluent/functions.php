...
if (!$gantry->isCompatible($requiredGantryVersion)) {
    ...
    $error = sprintf(__('Please upgrade Gantry 5 Framework to v%s (or later) before using %s theme!', 'g5_fluent'), strtoupper($requiredGantryVersion), $current_theme->get('Name'));
...
foreach ($helpers as $file) {
    if (!$filepath = locate_template($file)) {
        trigger_error(sprintf(__('Error locating %s for inclusion', 'g5_fluent'), $file), E_USER_ERROR);
    }

    require $filepath;
}

require_once(get_stylesheet_directory() . '/gf_functions.php');
